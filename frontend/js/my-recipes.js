document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const render = status => {
      ...
      document.querySelectorAll('[data-publish]').forEach(button => button.onclick = async () => { const result = await FoodLab.api('/api/recipes/' + button.dataset.publish, { method: 'PUT', body: { status: 'pending' } }); const item = data.items.find(x => String(x.id) === String(button.dataset.publish)); if (item) item.status = result.data.status; render('draft'); FoodLab.toast('已提交，请等待审核'); });
    };
