...
def my_recipes():
    ...
    if not uid:
        return jsonify(error="请先登录"), 401
    db = connect(current_app.config["DATABASE_PATH"])
    rows = [dict(x) for x in db.execute("SELECT id,title,description,cover_image,cuisine,difficulty,prep_time,cook_time,status,likes_count,favorites_count FROM recipes WHERE author_id=? ORDER BY updated_at DESC", (uid,)).fetchall()]
    db.close()
